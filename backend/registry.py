...
class Registry:
    def __init__(self, qdrant_client: QdrantClient = None):
        ...
        self.use_vector_storage = qdrant_client is not None
        
        # Initialize Qdrant collections if client is available
    ...
    def register_tool(self, tool_id: str, tool: BaseTool):
        ...
        if self.use_vector_storage and self.openai_client:
            try:
                ...
                print(f"Warning: Failed to store tool in vector database: {e}")
        
        # Always store in memory for development
        self.tools_storage[tool_uuid] = {
            "id": tool_id,
            "description": tool.description,
            "tool_uuid": tool_uuid
        }
        ...
        self.tools[tool_uuid] = (
            ...
            RemoteTool(
                ...
                tool_id=tool_uuid,
                description=tool.description,
                model_dict=tool.args_schema.model_json_schema() if hasattr(tool, 'args_schema') and tool.args_schema else {},
            ),
        )
    ...
    def find_tools(self, query: str):
        ...
        # Fallback to simple text search in memory
        results = []
        query_lower = query.lower()
        for tool_uuid, tool_data in self.tools_storage.items():
            if (query_lower in tool_data["id"].lower() or 
                query_lower in tool_data["description"].lower()):
                results.append({
                    "id": tool_uuid,
                    "payload": tool_data,
    ...
    def list_agents(self):
        if self.use_vector_storage:
            try:
                ...
                print(f"Warning: Failed to list agents from vector database: {e}")
        ...
        return [
            ...
            for agent_uuid, agent_data in self.agents_storage.items()
        ]
    ...
    def list_tools(self):
        if self.use_vector_storage:
            try:
                ...
                print(f"Warning: Failed to list tools from vector database: {e}")
